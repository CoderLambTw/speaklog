...
const Views = (() => {
  ...
  function grammarPairHtml(g, src = '') {
    ...
    return `<div class="gpair">
      ${g.wrong ? `<div class="gline wrong"><span class="mark">❌</span><span>${wrongHtml}</span></div>` : ''}
      <div class="gline right"><span class="mark">✔️</span><span>${correctHtml}</span></div>
    ...
    root.innerHTML = `<div class="page">
      ...
      <div class="card">
        ...
        <div class="task-row">
          ...
          <a class="btn sm" href="#/add">＋ 新增</a>
        </div>
      </div>

      <div class="spacer"></div>
      <div class="grid cols-4">
        <div class="card stat"><span class="n">${s.lessons.length}</span><span class="lbl">課程數</span></div>
        <div class="card stat"><span class="n">${vocab.length}</span><span class="lbl">累積單字</span></div>
        <div class="card stat"><span class="n">${mastered}</span><span class="lbl">已精熟單字</span></div>
        <div class="card stat"><span class="n">${Store.totalReviews()}</span><span class="lbl">總複習次數</span></div>
      </div>

      ${ins.length ? `<div class="section-title"><h2>💡 學習洞察</h2></div>
      ...
      <div class="section-title"><h2>🗓️ 學習活動</h2></div>
      <div class="card">${Charts.heatmap(s.meta.activity)}</div>

      <div class="section-title"><h2>🗣️ 最新一堂課</h2><a href="#/lessons">全部課程 →</a></div>
      <div class="card clickable" data-go="#/lesson/${last.id}">
        <div class="flex between wrap">
          <div><b>${fmtDate(last.date)} · ${esc(last.teacher)}</b>
          <div class="lesson-counts"><span>🟢 ${last.vocab.length} 單字</span><span>✏️ ${last.grammar.length} 文法</span><span>📣 ${last.pron.length} 發音</span></div></div>
        </div>
  ...
  function emptyHome(root) {
    root.innerHTML = `<div class="page"><div class="empty">
      ...
      <div class="btn-row" style="justify-content:center">
        <a class="btn primary lg" href="#/add">📝 貼上第一堂課筆記</a>
        <button class="btn lg" id="load-sample">🎬 先看範例資料</button>
      </div>
    </div></div>`;
...
You did a wonderful job expressing your opinions today!`;
  ...
  function add(root) {
    root.innerHTML = `<div class="page" style="max-width:760px">
      <div class="page-head"><div><h1>📝 新增課程筆記</h1><div class="sub">把老師課後給的筆記原封不動貼進來,系統會自動解析</div></div></div>
      <div class="card">
        ...
        <button class="btn primary block lg" id="parse-btn">🔍 解析筆記</button>
      </div>
      <div id="preview"></div>
    </div>`;
  ...
  function renderPreview(el, parsed, dateInput) {
    ...
    el.innerHTML = `<div class="spacer"></div><div class="card">
      ...
      <div class="preview-sec"><div class="sec-title">👩‍🏫 老師</div><div class="pv-item"><b>${esc(parsed.teacher || '(未偵測到)')}</b></div></div>
      <div class="preview-sec"><div class="sec-title">🟢 單字 <span class="cnt">${parsed.vocab.length}</span></div>
      ...
      <div class="preview-sec"><div class="sec-title">✏️ 文法修正 <span class="cnt">${parsed.grammar.length}</span></div>
        ${parsed.grammar.map((g) => grammarPairHtml({ ...g, cats: Grammar.classify(g.wrong, g.correct) })).join('') || '<div class="pv-item faint">無</div>'}</div>
      <div class="preview-sec"><div class="sec-title">📣 發音 <span class="cnt">${parsed.pron.length}</span></div>
      ...
      <div class="preview-sec"><div class="sec-title">💬 老師回饋</div><div class="pv-item" style="white-space:pre-wrap">${esc(parsed.feedback || '(無)')}</div></div>
      <button class="btn primary block lg" id="save-btn" ${total === 0 && !parsed.feedback ? 'disabled' : ''}>💾 儲存這堂課(+30 XP)</button>
    </div>`;
  ...
  function lessons(root) {
    ...
    root.innerHTML = `<div class="page">
      <div class="page-head"><div><h1>📚 課程紀錄</h1><div class="sub">共 ${s.lessons.length} 堂課</div></div>
      <a class="btn primary" href="#/add">＋ 新增課程</a></div>
        ...
        <div class="grid">${ls.map((l) => `
          <div class="card clickable lesson-card" data-go="#/lesson/${l.id}">
            ...
            <div class="lesson-info">
              ...
              <div class="lesson-counts"><span>🟢 ${l.vocab.length} 單字</span><span>✏️ ${l.grammar.length} 文法</span><span>📣 ${l.pron.length} 發音</span>${l.feedback ? '<span>💬 回饋</span>' : ''}</div>
            </div>
          </div>`).join('')}</div>`).join('')}
    </div>`;
    $$('[data-go]', root).forEach((el) => el.addEventListener('click', () => (location.hash = el.dataset.go)));
  ...
  function lessonDetail(root, id) {
    ...
    root.innerHTML = `<div class="page" style="max-width:760px">
      <div class="page-head"><div>
        ...
        <div class="sub">👩‍🏫 ${esc(l.teacher)}</div>
      </div><div class="btn-row">
        <a class="btn sm" href="#/lessons">← 返回</a>
        <button class="btn danger sm" id="del-lesson">🗑 刪除</button>
      </div></div>

      ${l.feedback ? `<div class="card"><h3>💬 老師回饋</h3><div class="fb-quote">${esc(l.feedback)}</div></div><div class="spacer"></div>` : ''}

      ${l.vocab.length ? `<div class="card list-card"><div style="padding:16px 16px 6px"><h3>🟢 單字(${l.vocab.length})</h3></div>
        ${l.vocab.map((v) => {
          ...
          return `<div class="vrow" data-speak="${esc(v.word)}">
            ...
            ${statusPill(card)}<button class="iconbtn" title="播放發音">🔊</button>
          </div>`;
        }).join('')}</div><div class="spacer"></div>` : ''}

      ${l.grammar.length ? `<div class="card list-card"><div style="padding:16px 16px 6px"><h3>✏️ 文法修正(${l.grammar.length})</h3></div>
        ${l.grammar.map((g) => grammarPairHtml(g)).join('')}</div><div class="spacer"></div>` : ''}

      ${l.pron.length ? `<div class="card list-card"><div style="padding:16px 16px 6px"><h3>📣 發音(${l.pron.length})</h3></div>
        ${l.pron.map((p) => `<div class="vrow" data-speak="${esc(p.word)}">
          ...
          <button class="iconbtn" title="播放發音">🔊</button>
        </div>`).join('')}</div>` : ''}
    </div>`;
  ...
  function vocab(root, _, query) {
    ...
    function renderList() {
      ...
      $('#vlist', root).innerHTML = list.length ? list.map((v) => `
        <div class="vrow" data-word="${esc(v.key)}">
          ...
          ${statusPill(v.card)}
          <button class="iconbtn" data-speak="${esc(v.word)}" title="播放發音">🔊</button>
    ...
    root.innerHTML = `<div class="page">
      <div class="page-head"><div><h1>📖 單字庫</h1><div class="sub">${all.length} 個單字,${counts.mastered} 個已精熟</div></div></div>
  ...
  function vocabModal(v) {
    ...
    App.modal(`
      <h3>${esc(v.word)} <button class="iconbtn" id="m-speak">🔊</button></h3>
      <p>${esc(v.def || '(無定義)')}</p>
  ...
  function review(root) {
    ...
    if (!due.length) {
      root.innerHTML = `<div class="page"><div class="empty">
        ...
        <div class="btn-row" style="justify-content:center">
          <a class="btn" href="#/vocab">逛逛單字庫</a><a class="btn" href="#/pron">練練發音 🎙️</a>
        </div></div></div>`;
      return;
    }
    // 排序:到期單字 → 文法 → 新字
    ...
    function render() {
      ...
      const front = item.type === 'vocab'
        ? `<div class="label">${item.card.state === 'new' ? '新單字' : '這個字是什麼意思?'}</div>
           ...
           <button class="iconbtn" data-speak="${esc(item.word)}" style="font-size:18px;width:42px;height:42px">🔊</button>`
        : `<div class="label">怎麼說才對?在心裡修正這句話</div>
           <div class="sentence">❌ ${esc(item.item.wrong)}</div>`;
      const back = item.type === 'vocab'
        ? `<div class="divider"></div><div class="answer">${esc(item.def || '(無定義)')}</div>`
        : `<div class="divider"></div><div class="sentence answer" style="color:var(--text)">✔️ ${Grammar.diffHtml(item.item.wrong, item.item.correct).correctHtml}</div>
           <div class="gmeta" style="justify-content:center">${catPills(item.item.cats)}</div>`;

      root.innerHTML = `<div class="page review-wrap">
        <div class="review-top">
          <a class="iconbtn" href="#/" title="結束">✕</a>
        ...
        <div class="fcard">${front}${session.revealed ? back : ''}</div>
    ...
    function renderSummary() {
      ...
      root.innerHTML = `<div class="page review-wrap"><div class="card summary">
        ...
        <div class="nums">
          <div><div class="n">${session.done}</div><div class="l">張卡片</div></div>
          <div><div class="n">${acc}%</div><div class="l">記得率</div></div>
          <div><div class="n">${Store.weekStreak()}</div><div class="l">週連勝 🔥</div></div>
        </div>
        <div class="btn-row" style="justify-content:center">
          <a class="btn primary" href="#/">回到總覽</a>
          <a class="btn" href="#/progress">看看成長 📈</a>
        </div>
      </div></div>`;
      newAch.forEach((a) => App.achToast(a));
      App.updateNavBadge();
  ...
  function grammar(root) {
    ...
    root.innerHTML = `<div class="page">
      <div class="page-head"><div><h1>🎯 文法雷達</h1><div class="sub">看見自己反覆犯的錯,才能真正改掉它</div></div></div>
      <div class="card">
        <h3>錯誤模式分佈</h3>
      ...
      <div class="section-title"><h2>📜 所有修正紀錄(${idx.length})</h2></div>
  ...
  function pron(root) {
    ...
    root.innerHTML = `<div class="page">
      <div class="page-head"><div><h1>🎙️ 發音練習室</h1>
      ...
      <div class="grid cols-2">
        ${list.map((p) => `<div class="card pron-card" data-key="${esc(p.key)}">
          ...
          <div class="pron-actions">
            ...
            <button class="btn sm" data-act="slow">🐢 慢速</button>
            ${SR ? '<button class="btn sm primary" data-act="mic">🎤 換我說</button>' : ''}
          </div>
  ...
  function progress(root) {
    ...
    root.innerHTML = `<div class="page">
      <div class="page-head"><div><h1>📈 成長軌跡</h1><div class="sub">Lv.${lv.level} ${lv.title} · 總計 ${s.meta.xp} XP</div></div></div>
      <div class="grid cols-2">
        <div class="card"><h3>📚 累積單字量</h3>${Charts.line(cumPoints)}</div>
        <div class="card"><h3>🎯 複習記得率(近 14 次)</h3>${accPoints.length >= 2 ? Charts.line(accPoints, { color: '#2dd4bf', color2: '#6c7bff', yFmt: (v) => v + '%' }) : '<div class="faint" style="text-align:center;padding:40px 0">完成幾次複習後,曲線就會出現</div>'}</div>
      </div>
      <div class="spacer"></div>
      <div class="grid cols-2">
        <div class="card"><h3>🗓️ 每月上課數</h3>${Charts.hbars(monthItems)}</div>
        <div class="card"><h3>🧠 單字記憶分佈</h3>${Charts.hbars([
      ...
      <div class="section-title"><h2>🏅 成就(${unlocked.size}/${Store.ACHIEVEMENTS.length})</h2></div>
      <div class="ach-grid">
  ...
  function settings(root) {
    ...
    root.innerHTML = `<div class="page" style="max-width:640px">
      <div class="page-head"><div><h1>⚙️ 設定</h1></div></div>
      <div class="card">
  ...
  function emptyRedirect(root, title, body) {
    root.innerHTML = `<div class="page"><div class="empty">
      ...
      <a class="btn primary lg" href="#/add">📝 新增課程筆記</a>
    </div></div>`;
  }
